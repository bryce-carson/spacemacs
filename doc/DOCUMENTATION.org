...
The concept of modes is already present in Emacs, so text editing modalities are
referred to as *states* in Spacemacs.

The [[https://github.com/syl20bnr/evil-tutor][evil-tutor]] package is intended to familiarize you with *modal* editing of
text with =Vim= modes (states). More information about the tutor itself is in
the [[file:QUICK_START.org][Quick Start Guide]] under its own heading.

Press ~SPC h T v~ when you are in /*Normal* state/ to begin using the tutor.
